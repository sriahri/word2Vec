...
def naiveSoftmaxLossAndGradient(
    ...
    y_cap = softmax(np.dot(outsideVectors, centerWordVec))
    y = np.zeros(y_cap.shape)
    y[outsideWordIdx] = 1
    loss = -np.log(y_cap[outsideWordIdx])
    
    gradCenterVec = np.dot(outsideVectors.transpose(), y_cap - y)
    gradOutsideVecs = np.outer(y_cap-y, centerWordVec)
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(np.dot(U_o.transpose(), V_c))) - np.sum(np.log(sigmoid(-np.dot(U_k, V_c))))
    gradCenterVec = -(1-sigmoid(np.dot(U_o.transpose(), V_c))) * U_o + np.dot((1-sigmoid(-np.dot(U_k, V_c))).transpose(),U_k)

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx, :] = ((sigmoid(np.dot(U_o.transpose(),V_c)) - 1)*V_c)

    unique, unique_indices, unique_counts = np.unique(negSampleWordIndices, return_counts= True, return_index= True)
    unique_samples = sigmoid(-np.dot(U_k, V_c))[unique_indices]
    gradOutsideVecs[unique, :] = -((unique_samples - 1) * unique_counts)[:, None] * (V_c[None, :])
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    center_word_vector = centerWordVectors[center_word_index]

    for word in outsideWords:
        current_loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(center_word_vector, word2Ind[word], outsideVectors, dataset)
        loss += current_loss
        gradCenterVecs[center_word_index] += gradCenterVec
